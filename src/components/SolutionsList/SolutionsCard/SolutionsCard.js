...
const SolutionsCard = (props) => {
  ...
  const normalizeAlgorithmName = (algorithmValue) => {
    const algorithmItem = GenAlgorithmsItems.find(
      ({ value }) => value === algorithmValue
    );
    if (algorithmItem !== undefined) return algorithmItem.title;
    else return algorithmValue;
  };

  return (
    <li className="solution-card" onClick={handleItemClicked}>
      <h3>{props.solveId}</h3>
      <h3>{props.mazeId}</h3>
      <h3>{props.maze.width}</h3>
      <h3>{props.maze.height}</h3>
      <h3>{normalizeAlgorithmName(props.maze.genAlgorithmType)}</h3>
      <h3>{props.solvAlgorithmType}</h3>
      <div className="compare-checkbox-wrapper">
        <input
          className="compare-checkbox"
          id="compare-checkbox-solutions-id"
          type="checkbox"
          onClick={handleCheckboxChange}
        />
      </div>
      <div className="delete-solution-button" onClick={handleDeleteClicked}>
